...
/* font constructor */
c_font::c_font( ) : height( 0 ), flags( 0 ), weight( 0 ), heighest_char( 0 ), dev( nullptr ), texture( nullptr ), vertex_buffer( nullptr ), text_width( 0 ), text_height( 0 ), text_scale( 0 ), spacing( 0 ), saved( nullptr ), draw( nullptr ), line_height( 0 ) { }
c_font::c_font( std::string font, uint32_t height, uint32_t weight, uint32_t highest_char, uint32_t flags ) : font( std::move( font ) ), height( height ), flags( flags ), weight( weight ), heighest_char( highest_char ), dev( nullptr ), texture( nullptr ), vertex_buffer( nullptr ), text_width( 0 ), text_height( 0 ), text_scale( 0 ), spacing( 0 ), saved( nullptr ), draw( nullptr ), line_height( 0 ) {
...
HRESULT c_font::init_device_objects( IDirect3DDevice9* dev ) {
	...
	const uint32_t hr = this->dev->CreateTexture( this->text_width, this->text_height, 1, D3DUSAGE_DYNAMIC, D3DFMT_A4R4G4B4, D3DPOOL_DEFAULT, &this->texture, nullptr );
	if ( FAILED( hr ) )
		return hr;
	...
	const auto bit_map = CreateDIBSection( dc, &bmi, DIB_RGB_COLORS, reinterpret_cast< void** >( &bitmap_bits ), nullptr, 0 );
	...
	const auto font = CreateFontA( height, 0, 0, 0, this->weight, this->flags & D3DFONT_ITALIC, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, this->font.c_str( ) );
...
HRESULT c_font::restore_device_objects( )
{
	...
	if ( FAILED( hr = this->dev->CreateVertexBuffer( max_vertices * sizeof font2dvertex, D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC, 0, D3DPOOL_DEFAULT, &this->vertex_buffer, nullptr ) ) )
		return hr;
